 ...
 * @version    $Id: Stream.php 1490 2020-06-23 15:23:06Z jan.slabon $
 */
    ...
    implements SetaPDF_Core_Reader_ReaderInterface
{
    ...
    public function reset($pos = 0, $length = 200)
    {
        ...
        if ($this->_length < $length && $this->increaseLength($length - $this->_length)) {
            ...
            $this->_buffer = substr($this->_buffer, 0, $length);
            $this->_length = strlen($this->_buffer);
        }
    }
    ...
    {
        if ($writer instanceof SetaPDF_Core_Writer_Stream) {
        ...
        $writer->write(stream_get_contents($this->_stream));
        fseek($this->_stream, $currentPos);
    }

    /**
     * Implementation of {@link SetaPDF_Core_Reader_ReaderInterface} (empty body for this type of reader).
     *
