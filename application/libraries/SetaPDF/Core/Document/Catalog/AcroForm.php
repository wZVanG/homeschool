 ...
 * @version    $Id: AcroForm.php 1465 2020-05-04 08:58:39Z jan.slabon $
 */
...
class SetaPDF_Core_Document_Catalog_AcroForm
{
    ...
    public function addDefaultEntriesAndValues()
    {
        ...
        if ($dictionary->offsetExists('DR') === false) {
            $dictionary->offsetSet('DR', new SetaPDF_Core_Type_Dictionary());
        }

        /** @var SetaPDF_Core_Type_Dictionary $drDictionary */
        $drDictionary = $dictionary->getValue('DR')->ensure(true);
        if (!$drDictionary->offsetExists('Font')) {
            $drDictionary->offsetSet('Font', new SetaPDF_Core_Type_Dictionary());
        }

        /** @var SetaPDF_Core_Type_Dictionary $fontDictionary */
        $fontDictionary = $drDictionary->getValue('Font')->ensure(true);
