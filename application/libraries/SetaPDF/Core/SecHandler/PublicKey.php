 ...
 * @version    $Id: PublicKey.php 1409 2020-01-30 14:40:05Z jan.slabon $
 */
    ...
    implements SetaPDF_Core_SecHandler_SecHandlerInterface
{
     ...
     */
    protected $_cipherId = OPENSSL_CIPHER_AES_256_CBC;
        ...
        try {
            ...
            foreach ($recipients AS $recipient) {
                ...
                if (!isset($permissionGroups[$recipient->getPermissions()])) {
                    $permissionGroups[$recipient->getPermissions()] = [];
                }
            ...
            foreach ($permissionGroups AS $permissions => $recipients) {
                ...
                $stat = openssl_pkcs7_encrypt(
                    $tmpFileIn, $tmpFileOut, $certificates, [], PKCS7_BINARY, $this->getCipherId()
                );
                ...
                if (false === $stat) {
                    ...
                    throw new SetaPDF_Core_SecHandler_Exception('OpenSSL error: ' . implode(', ', $errorMsgs));
                }
    ...
    {
        $data = $seed . implode('', $envelopes) . ($encryptMetadata ? '' : "\xFF\xFF\xFF\xFF");
    ...
    public function getPermissions()
    {
        ...
        if ($this->_authData['perm'] & self::PERM_OWNER) {
            return $all;
        }
