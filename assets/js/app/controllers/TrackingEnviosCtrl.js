angular.module('app').controller("TrackingEnviosCtrl", ["$scope", "API", "DoksUtils", "WAI", "$http", "$timeout", "$interval", "FileUploader", "$mdToast", "$state", "$filter", "Usuario", "Cliente", function($scope, API, DoksUtils, WAI, $http, $timeout, $interval, FileUploader, $mdToast, $state, $filter, Usuario, Cliente){
  ...
  $scope.exists = function (item, list) {
    ...
    $scope.validProducto = () => {
        return $scope.formulario.$valid && $scope.selected.length;
    }
    ...
    $scope.$watch("registro_info.tipo", (value) => {
    ...
    function busquedaProductos(){
        ...
        let params = Object.assign($scope.productos_filtros, {});
    ...
    $scope.productos_filtros = {
        id_local: 1,
        id_estado_envio: 1,
        id_categoria: 0
    };

    $scope.$watchCollection("productos_filtros",function(value){
        if(!value) return;
    ...
    function guardar(){
        ...
        let post_data = Object.assign({}, $scope.registro_info, {selected: $scope.selected});

        $http.post(API.url("tracking", "guardar_envio"), post_data).success(function(data){
